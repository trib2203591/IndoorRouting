...
export async function postFeature(featureType) {
    try {
        ...
        var bodyToPost = JSON.stringify(getFeatureBody(featureType))
        var featuretype = ''
        if (featureType == 'Address') {
            ...
            featuretype = featureType.toLowerCase().trim() + 's'
        }
        ...
        const tE = document.getElementById('testBody')
        tE.innerHTML = `<div>${bodyToPost}</div>`
    }
}
